...
- Mouse is optional: **on by default** (restored on quit/`kill`; watchdog covers hard kills). Disable with `rusticlens-tui --no-mouse` or `RUSTICLENS_NO_MOUSE=1`. While mouse mode is on, hold **Shift** to bypass it and use the terminal's native mouse — e.g. Tilix / GNOME Terminal right-click menu and select→copy (VTE terminals). Detail horizontal pan: Shift/Alt/Ctrl+wheel, wheel on the bottom ←→ bar, or trackpad side-swipe.
...
- **Copy needs a clipboard backend** — see [Clipboard (TUI yank / copy)](#clipboard-tui-yank--copy); with `--mouse`, the terminal’s native select→copy is unavailable while the TUI is open. Workarounds: hold **Shift** while selecting/right-clicking to bypass mouse reporting (VTE terminals such as Tilix and GNOME Terminal), or run with `--no-mouse`.
